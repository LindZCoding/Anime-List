...
router.get('/planToWatchResults', (req, res)=>{
    ...
    .then(apiData => {
        ...
        let planToWatchResults = apiData.data.results
        res.render('planToWatchResults', {planToWatchResults: planToWatchResults})
    })
    .catch(error => {
        console.error
    })
...
router.get('/:mal_id', (req, res) => {
    ...
    .then(idResults => {
        ...
        res.render('planToWatchDetail', {title_english: title_english, title: title, synopsis: synopsis, image_url: image_url, episodes: episodes, score: score, animeId: animeId})
    })
})
